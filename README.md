...
├── tokushoho.html         # 特定商取引法に基づく表記（ひな形・要編集）
├── privacy.html           # プライバシーポリシー（ひな形・要編集）
├── terms.html             # 利用規約（ひな形・要編集）
...
`tokushoho.html`（特定商取引法に基づく表記）・`privacy.html`（プライバシーポリシー）・`terms.html`（利用規約）のひな形を同梱しています。タイトル画面下部からもリンクされています。

**公開前に必ず対応してください：**
- 各ページ内の `【　】` で囲まれた箇所（事業者名・住所・連絡先メールアドレス等）を、実際の情報に置き換えてください
- 内容は一般的なテンプレートです。特に課金機能（IAP）を導入する場合は、公開前に一度、行政書士・弁護士等の専門家にご確認いただくことを強く推奨します
- 特定商取引法の表記は、個人事業主の場合「住所・電話番号は請求があった場合に遅滞なく開示する」形式が消費者庁のガイドラインで認められています（詳細は最新のガイドラインをご確認ください）
